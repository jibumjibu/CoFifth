    ...
    // Convert # and b to musical symbols
      ...
      chordList.forEach((chord, i) => {
        ...
        if (cellId) {
          ...
          if (cellId === "r3c1" || cellId === "r3c3") {
            cell.textContent = formattedChord;
          } else {
            cell.innerHTML = `<div class="index-label">${i+1}</div>${formattedChord}`;
          }
    ...
    buttons.forEach(button => {
      // Update button labels to show ♯ and ♭
      button.textContent = formatKeyLabel(button.textContent);
